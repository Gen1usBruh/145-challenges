...
// #include <stdio.h> /* should be in .c file, but freaks out there for no reason */
...
/**
 * @note 'max_align_t' is supported from C11
 */
union vector_meta {
    struct {
        size_t _size;
...
#define cvector_capacity(vec) \
    ((vec) ? ((union vector_meta *)vec)[-1]._capacity : (size_t)0)
...
#define cvector_size(vec) \
    ((vec) ? (((union vector_meta *)vec)[-1]._size) : (size_t)0)
...
#define cvector_free(vec)                        \
    do {                                         \
        if ((vec)) {                             \
            union vector_meta *p1 = &(((union vector_meta *)vec)[-1]); \
...
#define cvector_set_capacity(vec, size)                          \
    do {                                                         \
        if ((vec)) {                                             \
            ((union vector_meta *)vec)[-1]._capacity  = (size);   \
        }                                                        \
    } while (0);
...
#define cvector_set_size(vec, size)                         \
    do {                                                    \
        if ((vec)) {                                        \
            ((union vector_meta *)vec)[-1]._size = (size);   \
        }                                                   \
    } while (0);
...
#define cvector_grow(vec, count)                                                \
    do {                                                                        \
        const size_t cv_sz__ = (count) * sizeof(*(vec)) + (sizeof(union vector_meta)); \
        if ((vec)) {                                                            \
            /* Probably this also correct? -> union vector_meta *cv_p1__ = &(((union vector_meta *)vec)[-1].size); */                                                                    \
            union vector_meta *cv_p1__ = &(((union vector_meta *)vec)[-1]);                           \
            union vector_meta *cv_p2__ = cvector_clib_realloc(cv_p1__, (cv_sz__));         \
            assert(cv_p2__);                                                    \
            (vec) = (void *)(&cv_p2__[1]);                                      \
            cvector_set_capacity((vec), (count));                               \
        } else {                                                                \
            union vector_meta *cv_p__ = cvector_clib_malloc(cv_sz__);                      \
            assert(cv_p__);                                                     \
            (vec) = (void *)(&cv_p__[1]);                                       \
            cvector_set_capacity((vec), (count));                               \
